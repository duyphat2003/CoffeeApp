...
class _ProductListState extends State<ProductList> {
  ...
  ProductType? getProductTypeFromString(String input) {
    try {
      return ProductType.values.firstWhere(
        (e) => e.name.toLowerCase() == input.toLowerCase(),
      );
    } catch (e) {
      return null; // or throw error, or handle invalid input
    }
  }
  ...
  Future<void> LoadData() async {
    ...
    if (widget.productType != "") {
      productFiltered = await FirebaseDBManager.productService
          .getProductsByType(getProductTypeFromString(widget.productType)!);
    }
  }
        ...
        title: SizedBox(
          ...
          child: SearchAnchor(
            ...
            suggestionsBuilder: (context, controller) async {
              ...
              return List<ListTile>.generate(productSearchList.length, (index) {
                final item = productSearchList[index].name;
                return ListTile(
                  title: Text(item),
                  onTap: () {
                    setState(() {
                      controller.closeView(item); // Optionally fill text field
                    });
                  },
                );
              });
